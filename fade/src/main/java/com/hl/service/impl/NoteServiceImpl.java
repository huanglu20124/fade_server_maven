...
public class NoteServiceImpl implements NoteService {
	...
	//private RedisDao redisDao = BasicFactory.getFactory().getInstance(RedisDao.class);
		...
		if(size <= 20){
			    ...
			    if(update_list.size() == 0){
					...
					for(Note note : update_list){
					    image_list.add(noteDao.findAllImage(note.getNote_id()));
						tag_list.add(noteDao.findAllTag(note.getNote_id()));
						//24号新增，转发帖获取转发链的内容
						...
						//3号新增,热评数组
						comment_list.add(commentDao.getThreeComment(note.getNote_id()));
					}
			    	map.put(Const.RESULT, ConvertUtil.convertNote2ListMap(update_list,image_list,tag_list,comment_list,relay_list_map));
			    }
			}else{
                ...
                allocNote(note_list_merge, result_list, id_list, next);
		...
		if(type == Const.FANS){
			for(Map<String, Object> map : note_list){
				...
				if(isDie_fans == 1){
					...
					list_ans.add(noteDao.findNoteById(note_id,Const.ALL));
				}
			}
	...
	public void allocNote(List<Map<String, Object>>list_original,List<Map<String, Object>>result_list, List<Integer>id_list,int next){
		for(int q = next; q >= 0; q--){
			...
			if(q >= next-19){
				...
				if(isDie_fans == 1){
					...
					//活着的话则添加图片数组，标签数组
					List<Map<String, Object>>image_list = noteDao.findAllImage(note.getNote_id());
					List<Map<String, Object>>tag_list = noteDao.findAllTag(note.getNote_id());
					List<Map<String,Object>>comment_list = commentDao.getThreeComment(note.getNote_id());
					Map<Integer,Object>relay_list_map = new HashMap<>();
					if(note.getIsRelay() != 0){
						//对于转发帖，调用前面的方法	
						dealWithRelay(relay_list_map, note);
					}
					result_list.add(ConvertUtil.convertNote2Map(note,image_list,tag_list,comment_list,relay_list_map));
				}
	...
	@Override
	public String getSmallSectionHome(String bunch) {
		//小请求，得到的是过滤的、更新的数组 
		Gson gson = new Gson();
		Map<String, Object>map = new HashMap<>();
	...
	@Override
	public String getSectionDiscoverRecommond(HttpServletRequest request, Integer start,String hot_id) {
		//发现的推荐版块
		...
		List<Map<String, Object>>hot_note_array = (List<Map<String, Object>>) request.getServletContext().getAttribute(Const.HOT_NOTE_ARRAY);
		if(start == 0){
			//首次加载，要返回hot_id
...
//			Map<String,Object>relay_list_map = new HashMap<>();
	...
	@Override
	public String getOriginNote(Integer isRelay) {
		//请求得到原始帖的内容 (已废弃)
		Gson gson = new Gson();
		Map<String, Object>map = new HashMap<>();
		Note originNote = noteDao.findNoteById(isRelay,Const.ALL);
		if(originNote == null){
			map.put(Const.ERR, "0");		
		}else{
			List<Map<String, Object>>image_list = noteDao.findAllImage(isRelay);
			List<Map<String, Object>>tag_list = noteDao.findAllTag(isRelay);
			List<Map<String,Object>>comment_list = commentDao.getThreeComment(isRelay);
			Map<Integer,Object>relay_list_map = new HashMap<>();
			
			if(originNote.getIsRelay() != 0){
				//是转发帖的话
				dealWithRelay(relay_list_map, originNote);
			}		
			Map<String, Object> note_map = ConvertUtil.convertNote2Map(originNote,image_list,tag_list,comment_list,relay_list_map);
			//并把标签和图片加进去 
			map.put(Const.RESULT, note_map);
		}
		return gson.toJson(map);
	}
	...
	public String getTwentyRelay(Integer note_id, Integer start) {
		...
		if(list.size() == 0) map.put(Const.ERR, 0);
		else{
	...
	@Override
	public String getRelayDetail(Integer note_id) {
		...
		if(note == null){
			...
			List<Map<String,Object>>comment_list = commentDao.getThreeComment(note_id);
			Map<Integer, Object>relay_map = new HashMap<>();
			if(note.getIsRelay() != 0){
				//加入转发链
				dealWithRelay(relay_map, note);
			}
			map.put(Const.RESULT, ConvertUtil.convertNote2Map(note, image_list, tag_list,comment_list, relay_map));
		}
		return new Gson().toJson(map);
	}
